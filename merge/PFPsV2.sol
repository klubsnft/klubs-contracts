...
contract PFPsV2 is Ownable, IPFPsV2 {
    using SafeMath for uint256;
    ...
    function removeProposal(uint256 proposalId) onlyOwner external {
        delete proposals[proposalId];
    }
    ...
    mapping(address => bool) public mileageMode;

    function mileageOn(address addr) onlyManager(addr) external {
        mileageMode[addr] = true;
        emit MileageOn(addr);
    }

    function mileageOff(address addr) onlyManager(addr) external {
        mileageMode[addr] = false;
        emit MileageOff(addr);
    }
